appcan.define("slider", function($, exports, module) {
    ...
    SliderView.prototype = {
            ...
            if(self.option.hasCircle){
                 ...
                 if(index > self.option.itemCount){
                    ...
                    for(var i = 0;i < name.length;i++){
                        if(i == index){
                            name[i].style.cssText= 'margin-right : .3em;float: left;padding: 0.2em;background-color: #aaa;border-radius: 50%;';
                        }else{
                            name[i].style.cssText = 'margin-right : .3em;float: left;padding: 0.2em;background-color: rgba(255,255,255,0.5);border-radius: 50%;';
                        }
                    }
            }
        },
        ...
        set:function(data){
            ...
            if(self.option.hasCircle){
                self.Circle = $('<div class="label1" style="height:1.5em;width: 100%;position: absolute;left: 0px;bottom: 0px;"></div>');
                var dian = $('<div class="labelzan" style="margin:0 auto;top :.6em;right: .2em;float:right"></div>');
                var zamcon = $('<div class="labelcon1" name="labContent" style="margin-right : .3em;float: left;padding: 0.2em;background-color: #aaa;border-radius: 50%;"></div>');
                var zamcon1 = $('<div class="labelcon" name="labContent"></div>');
                for(var tt = 0;tt < self.option.itemCount;tt++){
                    if(tt == 0){
                        dian.append(zamcon);
                    }else{
                        dian.append('<div class="labelcon" name="labContent" style="margin-right : .3em;float: left;padding: 0.2em;background-color: rgba(255,255,255,0.5);border-radius: 50%;"></div>');
                    }
                }
